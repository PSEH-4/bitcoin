...
import com.sapient.bitcoin.GetConfiguration;
...
public class BitcoinRepositoryRestImpl implements BitcoinPriceRepository {
	...
	@Autowired
	private GetConfiguration configuration = new GetConfiguration();
	...
	public Currency getCurrency(String targetCurrency){
		...
		try {
			String content = content(
					configuration.getBitcoinCurrencyEndpoint(), targetCurrency);
			JsonNode node = OBJECT_MAPPER.readTree(content);
			JsonNode bpi = node.get("bpi");
			double usd = bpi.get("USD").get("rate_float").asDouble();
			double target = bpi.get(targetCurrency.toUpperCase())
					.get("rate_float").asDouble();
	...
	public List<Price> getPrice(String start, String end){
		...
		try {
			String content = content(configuration.getBitcoinApiEndpoint(),
					start, end);
	...
	public String content(String url, Object... param)
			throws BitcoinApiException {
		String content = "";
		try {
